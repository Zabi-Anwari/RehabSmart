"""Knee rehabilitation API routes."""
...
from schemas import (
    ...
    CombinedPredictRequest, CombinedPredictResponse,
    RehabMetricsOut,
)
from services.knee_pipeline import predict_imu, predict_skeleton, predict_combined
from ml.common.inference_schema import predict_single_imu as _predict_single_imu

logger   = logging.getLogger(__name__)
router   = APIRouter(prefix="/api/knee", tags=["knee"])

_IMU_EXPECTED_SAMPLES   = 50
_IMU_EXPECTED_CHANNELS  = 45
_SKEL_EXPECTED_FRAMES   = 30
...
@router.post("/imu/predict", response_model=IMUPredictResponse)
async def imu_predict(req: IMUPredictRequest):
    ...
    if window.shape[1] != _IMU_EXPECTED_CHANNELS:
        raise HTTPException(
            400,
            f"Expected {_IMU_EXPECTED_CHANNELS} channels, got {window.shape[1]}. "
            "Ensure your IMU sends ax/ay/az/gx/gy/gz for all 7-8 sensors."
        )

    try:
        result = predict_imu(window, fs=req.fs, model_name=req.model_name)
    except Exception as exc:
        logger.exception("IMU prediction failed")
...
# ── Single-sensor IMU endpoint (6-channel, rule-based) ───────────────────────

@router.post("/imu/single", response_model=IMUPredictResponse)
async def imu_single_predict(req: IMUPredictRequest):
    """Accept a single 6-channel IMU window (ax/ay/az/gx/gy/gz) and return
    rule-based quality metrics. No multi-sensor array required."""
    window = np.array(req.window, dtype=np.float32)
...
async def skeleton_predict(req: SkeletonPredictRequest):
    ...
    if frames.shape[1] != _SKEL_EXPECTED_JOINTS or frames.shape[2] != _SKEL_EXPECTED_COORDS:
        raise HTTPException(
            400,
            f"Expected (n_frames, 26, 3), got {frames.shape}."
        )
    if len(frames) < 5:
    ...
    try:
        ...
        logger.exception("Skeleton prediction failed")
        raise HTTPException(500, f"Inference error: {exc}") from exc
...
@router.post("/combined/predict", response_model=CombinedPredictResponse)
async def combined_predict(req: CombinedPredictRequest):
