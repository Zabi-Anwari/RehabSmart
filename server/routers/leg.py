...
from schemas import (
    ...
    IMULegPredictRequest, IMULegPredictResponse,
    SkeletonPredictRequest, SkeletonPredictResponse,
    LegCombinedPredictRequest, LegCombinedPredictResponse,
    RehabMetricsOut,
)
from services.leg_pipeline import predict_imu, predict_skeleton, predict_combined
from ml.common.inference_schema import predict_single_imu as _predict_single_imu

logger = logging.getLogger(__name__)
router = APIRouter(prefix="/api/leg", tags=["leg"])

_ALLOWED_IMU_CHANNELS = {48, 36}
...
async def imu_single_predict(req: IMUPredictRequest):
    """Single 6-channel IMU window — rule-based quality assessment, no array required."""
...
@router.post("/imu/predict", response_model=IMULegPredictResponse)
async def imu_predict(req: IMULegPredictRequest):
    window = np.array(req.window, dtype=np.float32)

    if window.ndim != 2:
        raise HTTPException(400, "window must be 2-D (n_samples, n_channels)")
    if window.shape[1] not in _ALLOWED_IMU_CHANNELS:
        raise HTTPException(
            400,
            f"Leg IMU window must have 48 (dataset11) or 36 (hugadb) channels, got {window.shape[1]}."
        )
    if len(window) < 5:
    ...
    try:
        ...
        logger.exception("Leg IMU prediction failed")
        raise HTTPException(500, f"Inference error: {exc}") from exc

    return IMULegPredictResponse(
...
async def skeleton_predict(req: SkeletonPredictRequest):
    ...
    try:
        ...
        logger.exception("Leg skeleton prediction failed")
        raise HTTPException(500, f"Inference error: {exc}") from exc
...
@router.post("/combined/predict", response_model=LegCombinedPredictResponse)
async def combined_predict(req: LegCombinedPredictRequest):
    ...
    try:
        ...
        logger.exception("Leg combined prediction failed")
        raise HTTPException(500, f"Inference error: {exc}") from exc
    ...
    return LegCombinedPredictResponse(
        imu=imu_out,
        skeleton=skel_out,
